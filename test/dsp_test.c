...
void adc_test(int argc, char **argv)
{
	uint32_t enable;
	mailbox_t mailbox;
...
void pcm_test(int argc, char **argv)
{
	int i;
	uint32_t rate;
	uint32_t *aud_ptr;
	...
	mailbox_set_param(&mailbox, MAILBOX_CMD_AUDIO_DAC_PCM_CHECK, ((uint32_t)aud_addr) - W_DSP_DMEM_64KB_BASE_ADDR, aud_len * 8, 0);
	mailbox_ctrl(CMD_MAILBOX_CPU2DSP_SEND, &mailbox);
}

void record2dac_cb(MAILBOX_TYPE_T type, mailbox_t *param)
{
	mailbox_t mailbox;
	...
	case MAILBOX_CMD_AUDIO_DAC_PCM_WRITE_DONE:
		...
		if (NULL == node) {
			rt_kprintf("using_list is empty\r\n");
			break;
		}
...
void record2dac(int argc, char **argv)
{
	int i;
	mailbox_t mailbox;
	...
	if (os_strcmp(argv[1], "start") == 0) {
		...
		mailbox_ctrl(CMD_MAILBOX_SET_CALLBACK, (void *)record2dac_cb);
		...
		rt_kprintf("NOT support command %s.\r\n", argv[1]);
}

void line_in(int argc, char **argv)
{
	uint32_t enable;
...
void adc_analog_gain(int argc, char **argv)
{
	uint32_t param;
	mailbox_t mailbox;

	if (argc < 2) {
		rt_kprintf("please input gain.\r\n");
		return;
	}
...
void adc_digital_gain(int argc, char **argv)
{
	uint32_t param;
	mailbox_t mailbox;
...
void adc_sample_rate(int argc, char **argv)
{
	uint32_t param;
	mailbox_t mailbox;

	if (argc < 2) {
		rt_kprintf("please input sample rate.\r\n");
		return;
	}
	param = dsp_atoi(argv[1]);
	sddev_control(SCTRL_DEV_NAME, CMD_SCTRL_AUDIO_PLL, &param);
	mailbox_set_param(&mailbox, MAILBOX_CMD_AUDIO_ADC_SAMPLE_RATE_SET, param, 0, 0);
	mailbox_ctrl(CMD_MAILBOX_CPU2DSP_SEND, &mailbox);
}

void dac_analog_gain(int argc, char **argv)
{
	uint32_t param;
...
void dac_digital_gain(int argc, char **argv)
{
	uint32_t param;
	mailbox_t mailbox;
...
void dac_sample_rate(int argc, char **argv)
{
	uint32_t param;
	mailbox_t mailbox;
...
#if (CFG_USB == 1 && CFG_USE_USB_HOST == 1)
void usb_mount(int argc, char **argv)
{
	...
	if (ret == 0) {
		mount_flag = 1;
		rt_kprintf("usb mount OK!\n");
	} else {
		mount_flag = 0;
		rt_kprintf("usb mount fail!\n");
	}
}

void usb_unmount(int argc, char **argv)
{
	int ret;

	ret = dfs_unmount("/udisk");
	if (ret == 0) {
		mount_flag = 0;
		rt_kprintf("usb unmount OK!\n");
	} else {
		mount_flag = 1;
		rt_kprintf("usb unmount fail!\n");
	}
}
...
static void usb_record_thread_main(void *arg)
{
	...
	while (record_flag) {
		...
		if (ret) {
			rt_kprintf("get usb record semaphore fail.\r\n");
			break;
		}
		...
		if (node) {
			for (; node != NULL; node = (dma_buffer_node *)node->header.next) {
				co_list_extract(&g_record_context.using_list, (struct co_list_hdr *)node);
				write(record_file, node->buffer, node->size);
				co_list_push_back(&g_record_context.free_list, (struct co_list_hdr *)node);
...
static void usb_record_cb_hdl(MAILBOX_TYPE_T type, mailbox_t *param)
{
	int ret;
	dma_buffer_node *node;
	...
	case MAILBOX_CMD_AUDIO_ADC_PCM_READ:
		...
		if (ret)
			rt_kprintf("set usb record semaphore fail.\r\n");
		break;
	default:
		rt_kprintf("%s:%d cmd=0x%x!\r\n", __FUNCTION__, __LINE__, param->cmd);
		break;
...
void record2usb(int argc, char **argv)
{
	int ret;
	uint32_t index = 0;
	mailbox_t mailbox;
	...
	if (rt_strcmp(argv[1], "start") == 0) {
		...
		if (ret) {
			rt_kprintf("create usb record semaphore fail.\r\n");
			return;
		}
		record_flag = 1;
		thread_flag = 1;
		...
		if (ret) {
			rt_kprintf("create usb record thread fail.\r\n");
			return;
		}
		...
		if (ret)
			rt_kprintf("delete usb record semaphore fail.\r\n");
...
static void usb_play_cb_hdl(MAILBOX_TYPE_T type, mailbox_t *param)
{
	...
	case MAILBOX_CMD_AUDIO_DAC_PCM_WRITE_DONE:
		node = (dma_buffer_node *)co_list_pick(&g_record_context.using_list);
		if (node == NULL) {
			rt_kprintf("%s:%d using list is empty!\r\n", __FUNCTION__, __LINE__);
			break;
		}
...
void usb_play(int argc, char **argv)
{
	int ret;
	uint32_t index = 0;
	...
	os_memset(file_name, 0, sizeof(file_name));
	if (argc > 1)
		sprintf(file_name, "/udisk/%s", argv[1]);
	else
		strcpy(file_name, "/udisk/record.pcm");

	record_file = open(file_name, O_RDWR);
	if (record_file < 0) {
		rt_kprintf("open %s fail.\r\n", file_name);
		return;
	}
	...
	/*set mailbox callback*/
	mailbox_ctrl(CMD_MAILBOX_SET_CALLBACK, (void *)usb_play_cb_hdl);
	...
	mailbox_ctrl(CMD_MAILBOX_CLEAR_CALLBACK, (void *)usb_play_cb_hdl);
...
MSH_CMD_EXPORT(pcm_test, pcm test);
MSH_CMD_EXPORT(record2dac, record2dac);
MSH_CMD_EXPORT(line_in, line in);
MSH_CMD_EXPORT(adc_analog_gain, adc_analog_gain);
MSH_CMD_EXPORT(adc_digital_gain, adc_digital_gain);
MSH_CMD_EXPORT(adc_sample_rate, adc_sample_rate);
MSH_CMD_EXPORT(dac_analog_gain, dac_analog_gain);
MSH_CMD_EXPORT(dac_digital_gain, dac_digital_gain);
MSH_CMD_EXPORT(dac_sample_rate, dac_sample_rate);
#if (CFG_USB == 1 && CFG_USE_USB_HOST == 1)
MSH_CMD_EXPORT(usb_mount, usb mount);
MSH_CMD_EXPORT(usb_unmount, usb unmount);
MSH_CMD_EXPORT(record2usb, record2usb);
MSH_CMD_EXPORT(usb_play, usb play);
#endif
#endif
