...
static rt_size_t rt_sound_read(rt_device_t dev, rt_off_t pos, void *buffer, rt_size_t size)
{
...
static rt_size_t rt_pcm_read(rt_uint8_t *buf, rt_uint32_t buf_size, void *buffer, rt_size_t size)
{
    rt_size_t read_bytes = 0;

    while (read_bytes + buf_size <= size) {
        memcpy((char *)buffer + read_bytes, buf, buf_size);
        read_bytes += buf_size;
    }
...
static void pcm_dac_record_thread_entry(void *parameter)
{
    uint8_t *mempool;
    rt_uint8_t *buffer;
    rt_uint32_t read_bytes = 0;
    rt_uint8_t *pcm_buf = NULL;
    rt_uint32_t pcm_size = 0;
    int rate;

    /* initialize mempool */
    micdac_loop->mp_block_size = 2048;
    micdac_loop->mp_cnt = 8;
    mempool = rt_malloc(micdac_loop->mp_block_size * micdac_loop->mp_cnt);
    rt_mp_init(&(micdac_loop->mp), "pcmdac_mp", mempool,
        micdac_loop->mp_block_size * micdac_loop->mp_cnt, micdac_loop->mp_block_size);

    rate = micdac_loop->sample_rate;
    switch (rate) {
        case 8000:
            pcm_buf = (rt_uint8_t *)PCM_8000;
            pcm_size = sizeof(PCM_8000);
            break;

        case 16000:
            pcm_buf = (rt_uint8_t *)PCM_16000;
            pcm_size = sizeof(PCM_16000);
            break;

        case 44100:
            pcm_buf = (rt_uint8_t *)PCM_44100;
            pcm_size = sizeof(PCM_44100);
            break;

        case 48000:
            pcm_buf = (rt_uint8_t *)PCM_48000;
            pcm_size = sizeof(PCM_48000);
            break;
    ...
    rt_kprintf("[pcmdac]:start loop back, tick %d \n", rt_tick_get());
    ...
    while (1) {
        ...
        if (!buffer) {
            ...
            /* read data from sound device */
            read_bytes = rt_pcm_read(pcm_buf, pcm_size, buffer, micdac_loop->mp_block_size);
            rt_device_write(micdac_loop->dac_device, 0, buffer, read_bytes);
        }
        ...
        if (micdac_loop->action == 0) {
            /* wait ack */
            rt_kprintf("[pcmdac]:stop loop back, tick = %d \n", rt_tick_get());
            break;
        }
    }
    rt_device_close(micdac_loop->dac_device);
...
mic_dac_exit:
    ...
    rt_kprintf("[pcmdac]:exit pcm dac, tick = %d \n", rt_tick_get());
}
...
static int mic_dac_loop(int argc, char **argv)
{
    ...
    else if (strcmp(argv[1], "pcm") == 0)
    {
        ...
        rt_kprintf("[pcmdac]:samplerate = %d \n", micdac_loop->sample_rate);

        /* create record thread */
        tid = rt_thread_create("pcm_dac",
                               pcm_dac_record_thread_entry,
                               RT_NULL,
                               1024 * 4,
                               27,
                               10);
    ...
    {
        int linein = atoi(argv[2]);
        if (!micdac_loop->adc_device) {
            rt_kprintf("audio mic not found \n");
            return 0;
        }
        rt_device_control(micdac_loop->adc_device, CODEC_CMD_LINEIN, (void *)&linein);
    }
