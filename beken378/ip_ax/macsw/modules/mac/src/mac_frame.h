...
/**
 * ieee80211_has_tods - check if MAC_FCTRL_TODS is set
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_has_tods(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TODS)) != 0;
}

/**
 * ieee80211_has_fromds - check if MAC_FCTRL_FROMDS is set
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_has_fromds(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_FROMDS)) != 0;
}

/**
 * ieee80211_has_a4 - check if MAC_FCTRL_TODS and MAC_FCTRL_FROMDS are set
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_has_a4(uint16_t fc)
{
    uint16_t tmp = cpu_to_le16(MAC_FCTRL_TODS | MAC_FCTRL_FROMDS);
    return (fc & tmp) == tmp;
}

/**
 * ieee80211_has_morefrags - check if MAC_FCTRL_MOREFRAG is set
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_has_morefrags(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_MOREFRAG)) != 0;
}

/**
 * ieee80211_has_retry - check if MAC_FCTRL_RETRY is set
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_has_retry(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_RETRY)) != 0;
}

/**
 * ieee80211_has_pm - check if MAC_FCTRL_PWRMGT is set
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_has_pm(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_PWRMGT)) != 0;
}

/**
 * ieee80211_has_moredata - check if MAC_FCTRL_MOREDATA is set
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_has_moredata(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_MOREDATA)) != 0;
}

/**
 * ieee80211_has_protected - check if MAC_FCTRL_PROTECTEDFRAME is set
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_has_protected(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_PROTECTEDFRAME)) != 0;
}

/**
 * ieee80211_has_order - check if MAC_FCTRL_ORDER is set
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_has_order(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_ORDER)) != 0;
}

/**
 * ieee80211_is_mgmt - check if type is MAC_FCTRL_MGT_T
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_mgmt(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK)) ==
           cpu_to_le16(MAC_FCTRL_MGT_T);
}

/**
 * ieee80211_is_ctl - check if type is MAC_FCTRL_CTRL_T
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_ctl(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK)) ==
           cpu_to_le16(MAC_FCTRL_CTRL_T);
}

/**
 * ieee80211_is_data - check if type is MAC_FCTRL_DATA_T
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_data(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK)) ==
           cpu_to_le16(MAC_FCTRL_DATA_T);
}

/**
 * ieee80211_is_data_qos - check if type is MAC_FCTRL_DATA_T and MAC_QOS_ST_BIT is set
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_data_qos(uint16_t fc)
{
    /*
     * mask with QOS_DATA rather than MAC_FCTRL_SUBT_MASK as we just need
     * to check the one bit
     */
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_QOS_ST_BIT)) ==
           cpu_to_le16(MAC_FCTRL_DATA_T | MAC_QOS_ST_BIT);
}

/**
 * ieee80211_is_data_present - check if type is MAC_FCTRL_DATA_T and has data
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_data_present(uint16_t fc)
{
     ...
     */
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | 0x40)) ==
           cpu_to_le16(MAC_FCTRL_DATA_T);
}

/**
 * ieee80211_is_assoc_req - check if MAC_FCTRL_MGT_T && MAC_FCTRL_ASSOCREQ_ST
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_assoc_req(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_MGT_T | MAC_FCTRL_ASSOCREQ_ST);
}

/**
 * ieee80211_is_assoc_resp - check if MAC_FCTRL_MGT_T && MAC_FCTRL_ASSOCRSP_ST
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_assoc_resp(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_MGT_T | MAC_FCTRL_ASSOCRSP_ST);
}

/**
 * ieee80211_is_reassoc_req - check if MAC_FCTRL_MGT_T && MAC_FCTRL_REASSOCREQ_ST
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_reassoc_req(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_MGT_T | MAC_FCTRL_REASSOCREQ_ST);
}

/**
 * ieee80211_is_reassoc_resp - check if MAC_FCTRL_MGT_T && MAC_FCTRL_REASSOCRSP_ST
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_reassoc_resp(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_MGT_T | MAC_FCTRL_REASSOCRSP_ST);
}

/**
 * ieee80211_is_probe_req - check if MAC_FCTRL_MGT_T && MAC_FCTRL_PROBEREQ_ST
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_probe_req(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_MGT_T | MAC_FCTRL_PROBEREQ_ST);
}

/**
 * ieee80211_is_probe_resp - check if MAC_FCTRL_MGT_T && MAC_FCTRL_PROBERSP_ST
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_probe_resp(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_MGT_T | MAC_FCTRL_PROBERSP_ST);
}

/**
 * ieee80211_is_beacon - check if MAC_FCTRL_MGT_T && MAC_FCTRL_BEACON_ST
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_beacon(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_MGT_T | MAC_FCTRL_BEACON_ST);
}

/**
 * ieee80211_is_atim - check if MAC_FCTRL_MGT_T && MAC_FCTRL_ATIM_ST
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_atim(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_MGT_T | MAC_FCTRL_ATIM_ST);
}

/**
 * ieee80211_is_disassoc - check if MAC_FCTRL_MGT_T && MAC_FCTRL_DISASSOC_ST
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_disassoc(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_MGT_T | MAC_FCTRL_DISASSOC_ST);
}

/**
 * ieee80211_is_auth - check if MAC_FCTRL_MGT_T && MAC_FCTRL_AUTHENT_ST
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_auth(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_MGT_T | MAC_FCTRL_AUTHENT_ST);
}

/**
 * ieee80211_is_deauth - check if MAC_FCTRL_MGT_T && MAC_FCTRL_DEAUTHENT_ST
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_deauth(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_MGT_T | MAC_FCTRL_DEAUTHENT_ST);
}

/**
 * ieee80211_is_action - check if MAC_FCTRL_MGT_T && MAC_FCTRL_ACTION_ST
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_action(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_MGT_T | MAC_FCTRL_ACTION_ST);
}

/**
 * ieee80211_is_back_req - check if MAC_FCTRL_CTRL_T && MAC_FCTRL_BAR_ST
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_back_req(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_CTRL_T | MAC_FCTRL_BAR_ST);
}

/**
 * ieee80211_is_back - check if MAC_FCTRL_CTRL_T && MAC_FCTRL_BA_ST
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_back(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_CTRL_T | MAC_FCTRL_BA_ST);
}

/**
 * ieee80211_is_pspoll - check if MAC_FCTRL_CTRL_T && MAC_FCTRL_PSPOLL_ST
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_pspoll(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_CTRL_T | MAC_FCTRL_PSPOLL_ST);
}

/**
 * ieee80211_is_rts - check if MAC_FCTRL_CTRL_T && MAC_FCTRL_RTS_ST
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_rts(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_CTRL_T | MAC_FCTRL_RTS_ST);
}

/**
 * ieee80211_is_cts - check if MAC_FCTRL_CTRL_T && MAC_FCTRL_CTS_ST
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_cts(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_CTRL_T | MAC_FCTRL_CTS_ST);
}

/**
 * ieee80211_is_ack - check if MAC_FCTRL_CTRL_T && MAC_FCTRL_ACK_ST
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_ack(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_CTRL_T | MAC_FCTRL_ACK_ST);
}

/**
 * ieee80211_is_cfend - check if MAC_FCTRL_CTRL_T && MAC_FCTRL_CFEND_ST
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_cfend(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_CTRL_T | MAC_FCTRL_CFEND_ST);
}

/**
 * ieee80211_is_cfendack - check if MAC_FCTRL_CTRL_T && MAC_FCTRL_CFEND_CFACK_ST
 * @fc: frame control bytes in little-endian byteorder
 */
static inline bool ieee80211_is_cfendack(uint16_t fc)
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_CTRL_T | MAC_FCTRL_CFEND_CFACK_ST);
}
...
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_DATA_T | MAC_NODATA_ST_BIT);
}
...
{
    return (fc & cpu_to_le16(MAC_FCTRL_TYPE_MASK | MAC_FCTRL_SUBT_MASK)) ==
           cpu_to_le16(MAC_FCTRL_DATA_T | MAC_QOS_ST_BIT | MAC_NODATA_ST_BIT);
}
...
{
    return (seq_ctrl & cpu_to_le16(MAC_SEQCTRL_FRAG_MSK)) == 0;
}
 ...
 */
static inline bool ieee80211_is_frag(struct mac_hdr *hdr)
{
    return ieee80211_has_morefrags(hdr->fctl) ||
           hdr->seq & cpu_to_le16(MAC_SEQCTRL_FRAG_MSK);
}
 ...
 */
static inline bool _ieee80211_is_robust_mgmt_frame(struct mac_hdr *hdr)
{
    if (ieee80211_is_disassoc(hdr->fctl) ||
        ieee80211_is_deauth(hdr->fctl))
        return true;

    if (ieee80211_is_action(hdr->fctl)) {
        uint8_t *category;
         ...
         */
        if (ieee80211_has_protected(hdr->fctl))
            return true;
        category = ((uint8_t *) hdr) + 24;
        return *category != MAC_PUBLIC_ACTION_CATEGORY &&
            *category != MAC_HT_ACTION_CATEGORY &&
            *category != MAC_UNPROT_WNM_ACTION_CATEGORY &&
            *category != MAC_SELF_PROT_ACTION_CATEGORY &&
            *category != MAC_VHT_ACTION_CATEGORY &&
            *category != MAC_VENDOR_ACTION_CATEGORY;
    }
 ...
 */
static inline bool ieee80211_is_robust_mgmt_frame(uint8_t *mgmt_frame, int len)
{
    if (len < MAC_SHORT_MAC_HDR_LEN)
        return false;
    return _ieee80211_is_robust_mgmt_frame((struct mac_hdr *)mgmt_frame);
}
