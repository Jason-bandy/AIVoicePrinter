...
/**
 * @name Frame Information for NPDA
 */
/**
...
/// @name VHT NDP Announcement frame format (see 802.11ac-2013, section 8.3.1.20)
...
/**
 * @name Frame Information for BRP (Beamforming Report Poll)
 */
/**
...
/// @name Beamforming Report Poll frame format (see 802.11ac-2013, section 8.3.1.21)
/// Offset of Retransmission Bitmap field
#define BFR_BRP_RETX_BITMAP_OFFSET          (16)

/**
 * @name Frame Information for VHT MIMO Control field
 */
 ...
 * @return true if beamformer role is enabled, false otherwise.
 ****************************************************************************************
 */
...
__INLINE bool bfr_is_calibrated(uint8_t sta_idx)
{
    ...
    return ((bfr->status & CO_BIT(BFR_STA_HAS_VALID_REPORT)) != 0);
}

/**
 ****************************************************************************************
 * @brief Return if a frame pointed by the given TX descriptor can be beamformed if
 * transmitted as a singleton MPDU
 *
 * @param[in] txdesc TX Descriptor to be checked
 *
 * @return true if the frame could be beamformed when sent as a singleton MPDU, false
 * otherwise
 ****************************************************************************************
 */
__INLINE bool bfr_is_bfmed_sglt_allowed(struct txdesc *txdesc)
{
    ...
    {
        return ((txdesc->umac.buf_control->tx_flags & TX_SWDESC_UMAC_TRIAL_BEAMFORM_BIT) != 0);
    }
    else
    {
        return ((txdesc->umac.buf_control->tx_flags & TX_SWDESC_UMAC_BEAMFORM_BIT) != 0);
    }
    #else
    return ((txdesc->umac.tx_flags & TX_SWDESC_UMAC_BEAMFORM_BIT) != 0);
    #endif
}
 ...
 * @param[in] host_length   Length allocated by the host for the reports
 ...
 * @param[in] rxdesc        RX Descriptor containing all the details about the received
 *                          frame
...
/**
 ****************************************************************************************
 * @brief Returns the latest Nc value received in a Beamforming report for a given beamformee.
 *
 * @param[in] sta_idx     STA Index of the Beamformee device
 * @return latest NC value
 ****************************************************************************************
 */
