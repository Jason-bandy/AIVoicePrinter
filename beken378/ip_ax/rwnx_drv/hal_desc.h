...
#define RWNX_WD_IS_AMPDU(whichdesc) \
    (((whichdesc) & __WD_AMPDU_BAPOL) == __WD_AMPDU_BAPOL)
#define RWNX_WD_IS_FIRST(whichdesc) \
    ((whichdesc) == __WD_AMPDU_FIRST)
#define RWNX_WD_IS_LAST(whichdesc) \
    ((whichdesc) == __WD_AMPDU_LAST)

/* c.f RW-WLAN-nX-MAC-HW-UM */
union rwnx_thd_phy_ctrl_info {
    struct {
        u32 soundingTx       : 1;
        u32 smoothingTx      : 1;
...
/* c.f RW-WLAN-nX-MAC-HW-UM */
union rwnx_thd_mac_ctrl_info_1 {
    struct {
        u32 rsvd1        : 9;
        u32 expectedAck  : 2;
        u32 lstp         : 1;
 ...
 */
/* c.f RW-WLAN-nX-LMAC-SW-UM */
struct rwnx_hw_txhdr {
    struct tx_policy_tbl policy;
    union rwnx_thd_mac_ctrl_info_1 mac_ctrl_info;
    union rwnx_thd_phy_ctrl_info phy_ctrl_info;
