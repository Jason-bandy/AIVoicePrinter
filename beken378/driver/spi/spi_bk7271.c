...
static void spi_slave_release_int_enable(UINT32 enable)
{
	UINT32 value;
...
static void spi_init_msten(UINT8 param)
{
	...
	REG_WRITE(SPI_CTRL, value);
	if (msten == 0)
		spi_slave_release_int_enable(1);
	else
		spi_slave_release_int_enable(0);
...
UINT32 spi_ctrl(UINT32 cmd, void *param)
{
	...
	case CMD_SPI_RXTRANS_EN:
		...
		break;
	case CMD_SPI_CS_EN:
		spi_slave_release_int_enable(*(UINT32 *)param);
		break;
	default:
		ret = SPI_FAILURE;
		break;
