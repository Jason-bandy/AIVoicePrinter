...
#if BK_MAC
/// Struct special frame format
struct phy_ppdu_format
{
    uint8_t     bw_mode ;
    uint8_t     chan_idx;
    uint8_t     modf    ;   //0:NON-HT; 1:HT-20M;  2:HT-40M; 3:HE
    uint8_t     mcs     ;   
    uint8_t     pre_gi  ;   //0:short preamble/gi;  1:long preamble/gi
    uint8_t     pld_type;   //0: all zeros;  1:incr; 2:rand
    uint8_t     tx_pwr  ;   //tx power      
    uint16_t    len     ;   //0 is inf
    uint16_t    num     ;   //0 is inf  
};
#endif
