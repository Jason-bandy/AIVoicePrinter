#ifndef _FATFS
#define _FATFS	3252	/* Revision ID */
...
#define LD2PD(vol) (uint8)(vol)	/* Each logical drive is bound to the same physical drive number */
...
FRESULT f_read (FIL* fp, void* buff, uint32 btr, uint32* br);	/* Read data from a file */
FRESULT f_wav_read(FIL * fp, void * buff, uint32 btr, uint32 * br);/* Read data from a file */

FRESULT f_mount (uint8 vol,FATFS* fs);	/* Mount/Unmount a logical drive */
...
WCHAR ff_convert (WCHAR chr, uint32 dir);	/* OEM-Unicode bidirectional conversion */
