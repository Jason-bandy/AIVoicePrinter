...
#if defined(CONFIG_TWS)||(CONFIG_APP_TOOLKIT_5 == 1)
...
#ifdef CONFIG_TWS
#define APP_WAVE_FILE_ID_STEREO_CONN		APP_WAVE_FILE_ID_RESERVED0
#define APP_WAVE_FILE_ID_STEREO_DISCONN		APP_WAVE_FILE_ID_RESERVED1
#define APP_WAVE_FILE_ID_STEREO_MATCH		APP_WAVE_FILE_ID_RESERVED2
#else
...
typedef struct _system_s
{
    ...
    uint8  button_mode_adc_enable;
    uint8  button_mode_adc_chan;
    uint8  button_mode_adc_sum; 
...
extern app_handle_t app_get_sys_handler(void);
...
#else
void app_bt_enable_sniff_mode( uint8 enable,uint8 set_link_policy);
#endif
void app_bt_enable_complete_wrap( hci_unit_t *unit );

#if 0
void app_bt_create_con_status_wrap( uint8_t status);
void app_bt_con_compl_wrap(hci_link_t * link, const btaddr_t  *rbtaddr );
#endif
...
void app_bt_flag_set( uint32 flag, uint8_t oper );
int app_bt_is_flag_set( uint32 flag );
...
#if 0
void app_bt_autoconn_start( void );
...
uint32 get_bt_mode(void);
void set_bt_mode(uint32);
...
void app_bt_flag2_set( uint32 flag, uint8 oper );
int app_bt_is_flag2_set( uint32 flag );
void system_mode_shift_for_sd_change(void);
void app_low_power_detect(void );
int app_button_powerdown( void );

#if 0
void app_bt_poweron_handler(void);
#endif
