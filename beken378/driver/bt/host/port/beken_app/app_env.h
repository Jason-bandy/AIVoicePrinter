...
#define ENV_BLOCK_LEN                    256
	...
	#if(use_4M_flash == 1)
	#define FLASH_ENVDATA2_DEF_ADDR 		(512*1024-0x1000)
	#else
	#define FLASH_ENVDATA2_DEF_ADDR 		(1024*1024-0x1000)
	#endif
#define FLASH_ENVDATA_DEF_ADDR		 (FLASH_ENVDATA2_DEF_ADDR - 0x1000)
#define FLASH_CHANGED_BT_ADDR      		(FLASH_ENVDATA_DEF_ADDR - 0x1000)
#if (CONFIG_APP_TOOLKIT_5 == 1)
	#if(use_4M_flash == 1)
	#define FLASH_ENVCFG_DEF_ADDR			  0x75000 //
	#else
	#define FLASH_ENVCFG_DEF_ADDR			  0xed800 //
	#endif
#elif defined(BT_DUALMODE)
#define FLASH_ENVCFG_DEF_ADDR             (500*1024) //(384*1024)
#else
#define FLASH_ENVCFG_DEF_ADDR             (480*1024) //(384*1024)
#endif
...
/*
typedef struct _app_env_write_req_s
{
    uint32  regId;   //( oper|addr)
    uint8 *data;
    uint8  len;
    STAILQ_ENTRY(_app_env_write_req_s) next;
}app_env_write_req_t;
*/
#ifdef APP_ENV_RESTART_MUSIC_RECOVER
extern uint8 exception_restart_flag;
extern uint8 app_env_music_flag_original_save;
#endif
...
extern env_saradc_cali_data_t * app_get_env_saradc_cali_data(void);
extern env_charge_cali_data_t * app_get_env_charge_cali_data(void);
extern env_aud_dc_offset_data_t* app_get_env_dc_offset_cali(void);
...
#ifdef APP_ENV_RESTART_MUSIC_RECOVER
void app_env_a2dp_write_flash( void);
void app_env_set_sw_power_down_flag(uint8 value);
uint8 app_env_get_music_paly_pause( btaddr_t *addr);
#endif
#ifdef CONFIG_TWS
...
#if (CONFIG_APP_TOOLKIT_5 == 1)&&(CONFIG_SW_SWITCH_KEY_POWER == 1)
