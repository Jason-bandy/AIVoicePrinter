...
SD_STATIC sdcard_rw_state_t sd_card_check_continious_rw(sdcard_rw_ops_t ops, uint32_t addr, uint32_t blk_cnt);
#endif
...
#if CONFIG_SDIO_V2P0
SD_STATIC beken_mutex_t s_mutex_sdcard;
...
SD_STATIC volatile sd_card_sw_status_t s_sdcard_sw_status;
#endif
#endif
SD_STATIC bool s_sd_card_is_init = false;
SD_STATIC sd_card_obj_t s_sd_card_obj = {0};

#if CONFIG_SDCARD_CHECK_INSERTION_EN
SD_STATIC bool sd_card_get_insert_status(void)
{
...
SD_STATIC uint32 sd_card_get_cmd_timeout_param(void)
{
    uint32 timeout_param = 0;
    ...
    case SDIO_HOST_CLK_XTAL_128DIV:
        timeout_param = CMD_TIMEOUT_203K;
        break;

    case SDIO_HOST_CLK_XTAL_1DIV:
        timeout_param = CMD_TIMEOUT_26_M;
        break;

    case SDIO_HOST_CLK_XTAL_2DIV:
        timeout_param = CMD_TIMEOUT_13_M;
        break;
    ...
    case SDIO_HOST_CLK_DCO_1DIV:
        timeout_param = CMD_TIMEOUT_120_M;
        break;

    case SDIO_HOST_CLK_DCO_2DIV:
        timeout_param = CMD_TIMEOUT_60_M;
        break;

    case SDIO_HOST_CLK_DCO_4DIV:
        timeout_param = CMD_TIMEOUT_30_M;
        break;

    case SDIO_HOST_CLK_DCO_128DIV:
        timeout_param = CMD_TIMEOUT_937K;
        break;
        #else
...
SD_STATIC uint32 sd_card_get_data_timeout_param(void)
{
    uint32 timeout_param = 0;
    ...
    switch (s_sd_card_obj.clock_freq) {
        ...
        #if (CFG_SOC_NAME == SOC_BK7252N)
    case SDIO_HOST_CLK_XTAL_128DIV:
        timeout_param = DATA_TIMEOUT_203K;
        break;

    case SDIO_HOST_CLK_XTAL_1DIV:
        timeout_param = DATA_TIMEOUT_26_M;
        break;

    case SDIO_HOST_CLK_XTAL_2DIV:
        timeout_param = DATA_TIMEOUT_13_M;
        break;
    ...
    case SDIO_HOST_CLK_DCO_1DIV:
        timeout_param = DATA_TIMEOUT_120_M;
        break;

    case SDIO_HOST_CLK_DCO_2DIV:
        timeout_param = DATA_TIMEOUT_60_M;
        break;

    case SDIO_HOST_CLK_DCO_4DIV:
        timeout_param = DATA_TIMEOUT_30_M;
        break;

    case SDIO_HOST_CLK_DCO_128DIV:
        timeout_param = DATA_TIMEOUT_937K;
        break;
        #else
    case SDIO_HOST_CLK_100K:
...
/* CMD0 */
SD_STATIC bk_err_t sd_card_cmd_go_idle_state(void)
{
...
/* CMD8 */
SD_STATIC bk_err_t sd_card_cmd_send_if_cond(void)
{
...
/* CMD55 */
SD_STATIC bk_err_t sd_card_cmd_app_cmd(uint32_t argument)
{
    bk_err_t error_state = BK_OK;
    sdio_host_cmd_cfg_t cmd_cfg = {0};
 ...
 */
SD_STATIC bk_err_t sd_card_cmd_sd_send_op_cond(uint32_t ocr)
{
    bk_err_t error_state = BK_OK;
    sdio_host_cmd_cfg_t cmd_cfg = {0};
...
/* CMD2 */
SD_STATIC bk_err_t sd_card_cmd_all_send_cid(void)
{
    bk_err_t error_state = BK_OK;
    sdio_host_cmd_cfg_t cmd_cfg = {0};
...
/* CMD3 */
SD_STATIC bk_err_t sd_card_cmd_send_relative_addr(uint16_t *rca)
{
    bk_err_t error_state = BK_OK;
    uint32_t rsp_arg = 0;
    sdio_host_cmd_cfg_t cmd_cfg = {0};
...
/* CMD9 */
SD_STATIC bk_err_t sd_card_cmd_send_csd(uint32_t argument)
{
    bk_err_t error_state = BK_OK;
    sdio_host_cmd_cfg_t cmd_cfg = {0};
...
/* CMD7 */
SD_STATIC bk_err_t sd_card_cmd_select_deselect_card(uint32_t addr)
{
    bk_err_t error_state = BK_OK;
    sdio_host_cmd_cfg_t cmd_cfg = {0};
    uint32_t retry_cnt = SD_CARD_MAX_CMD_TRIAL_COUNT;
 ...
 */
SD_STATIC bk_err_t sd_card_cmd_set_bus_width(bool four_lines)
{
    bk_err_t error_state = BK_OK;
    sdio_host_cmd_cfg_t cmd_cfg = {0};
...
/* CMD16 */
SD_STATIC bk_err_t sd_card_cmd_set_block_len(uint32_t block_size)
{
    bk_err_t error_state = BK_OK;
    sdio_host_cmd_cfg_t cmd_cfg = {0};
...
/* CMD25 */
SD_STATIC bk_err_t sd_card_cmd_write_multiple_block(uint32_t write_addr)
{
    bk_err_t error_state = BK_OK;
    sdio_host_cmd_cfg_t cmd_cfg = {0};
    uint32_t retry_cnt = SD_CARD_MAX_CMD_TRIAL_COUNT;
...
#else
/* CMD24 */
SD_STATIC bk_err_t sd_card_cmd_write_single_block(uint32_t write_addr)
{
    bk_err_t error_state = BK_OK;
    sdio_host_cmd_cfg_t cmd_cfg = {0};
...
/* CMD18 */
SD_STATIC bk_err_t sd_card_cmd_read_multiple_block(uint32 addr)
{
    bk_err_t error_state = BK_OK;
    sdio_host_cmd_cfg_t cmd_cfg = {0};
    uint32_t retry_cnt = SD_CARD_MAX_CMD_TRIAL_COUNT;
...
#else
/* CMD17 */
SD_STATIC bk_err_t sd_card_cmd_read_single_block(uint32 addr)
{
    bk_err_t error_state = BK_OK;
    sdio_host_cmd_cfg_t cmd_cfg = {0};
...
/* CMD32 */
SD_STATIC bk_err_t sd_card_cmd_set_erase_start_addr(uint32_t start_addr)
{
    bk_err_t error_state = BK_OK;
    sdio_host_cmd_cfg_t cmd_cfg = {0};
...
/* CMD33 */
SD_STATIC bk_err_t sd_card_cmd_set_erase_end_addr(uint32_t end_addr)
{
    bk_err_t error_state = BK_OK;
    sdio_host_cmd_cfg_t cmd_cfg = {0};
...
/* CMD38 */
SD_STATIC bk_err_t sd_card_cmd_erase(void)
{
    bk_err_t error_state = BK_OK;
    sdio_host_cmd_cfg_t cmd_cfg = {0};
...
/* CMD13 */
SD_STATIC bk_err_t sd_card_cmd_send_status(uint32_t argument, uint32_t *card_status)
{
    bk_err_t error_state = BK_OK;
    sdio_host_cmd_cfg_t cmd_cfg = {0};
...
SD_STATIC bk_err_t sd_card_power_on(void)
{
...
static inline void sdio_host_set_sample_edge(int edge)
{
    if(edge == 0)
        *(volatile unsigned int *)(SOC_SDIO_REG_BASE + 0x11*4) &= ~(1 << 26);
    else
        *(volatile unsigned int *)(SOC_SDIO_REG_BASE + 0x11*4) |= (1 << 26);
}

SD_STATIC bk_err_t sd_card_init_card(void)
{
    bk_err_t error_state = BK_OK;
    uint16_t sd_rca = 1;
    ...
    sdio_host_set_sample_edge(0);//set back to falling edge when working at high-freq clk for hardware reason
    rtos_delay_milliseconds(2);
    /* improve sdio clock freq for sd card data transfer mode */
    #if CONFIG_SDIO_V2P0
    s_sd_card_obj.clock_freq = SDIO_HOST_CLK_DCO_4DIV;
...
bk_err_t bk_sd_card_init(void)
{
    ...
    sdio_host_set_sample_edge(1);//set to rising edge according to sd protocol, a Toshiba 32G card cannot be recognized
...
SD_STATIC void sdcard_dump_transfer_data(UINT8 *write_buff, uint32_t first_block, uint32_t cnt)
{
    uint32_t i = 0;
...
SD_STATIC sdcard_rw_state_t sd_card_check_continious_rw(sdcard_rw_ops_t ops, uint32_t addr, uint32_t blk_cnt)
{
    ...
    SD_STATIC sdcard_rw_state_t s_baked_state = SDCARD_RW_STATE_INITIALIZED;	//baked the state of "complete current operation"
    SD_STATIC uint32_t s_baked_addr = 0xffffffff;	//invalid last address
    GLOBAL_INT_DECLARATION();

    #if CONFIG_CONT_RW_DEBUG
    SD_STATIC uint32_t s_continious_cnt = 0;
    SD_STATIC uint32_t s_total_cnt = 0;
    #endif
...
bk_err_t bk_sd_card_write_blocks(const uint8_t *data, uint32_t block_addr, uint32_t block_num)
{
    ...
    icu_ctrl(CMD_CLK_PWR_UP, &param); /*sys_drv_dev_clk_pwr_up(CLK_PWR_ID_SDIO, true);*/

    SD_CARD_LOGD("sd_card_check_continious_rw\r\n");
    if(sd_card_check_continious_rw(SDCARD_OPS_WRITE, block_addr, block_num) != SDCARD_RW_STATE_WRITING)
    {
        ...
        SD_CARD_LOGI("bk_sdio_host_reset_sd_state\r\n");
        bk_sdio_host_reset_sd_state();

        //TODO:just use multi-block mode
        SD_CARD_LOGI("sd_card_cmd_write_multiple_block\r\n");
    ...
    SD_CARD_LOGD("bk_sdio_host_config_data\r\n");
    bk_sdio_host_config_data(&data_config);

    SD_CARD_LOGD("bk_sdio_host_write_fifo\r\n");
...
bk_err_t bk_sd_card_read_blocks(uint8_t *data, uint32_t block_addr, uint32_t block_num)
{
    ...
    SD_CARD_LOGI("read[+]:rx data=0x%x,block_addr=0x%x,block_cnt=%d\r\n", data, block_addr, block_num);

    uint32_t param = PWD_SDIO_CLK;
    icu_ctrl(CMD_CLK_PWR_UP, &param); /*sys_drv_dev_clk_pwr_up(CLK_PWR_ID_SDIO, true);*/

    //check and do (send/no-send) multi-read CMD
    rw_state = sd_card_check_continious_rw(SDCARD_OPS_READ, block_addr, block_num);
    if(rw_state != SDCARD_RW_STATE_READING)
    {
        extern void bk_sdio_host_discard_previous_receive_data_sema(void);

        SD_CARD_LOGI("bk_sdcard_wait_busy_to_idle\r\n");
        error_state = bk_sdcard_wait_busy_to_idle(1000);
        if (error_state != BK_OK) {
            SD_CARD_LOGE("****sdcard is busy:0x%x\r\n", error_state);
            rtos_unlock_mutex(&s_mutex_sdcard);
            return error_state;
        }
        ...
        SD_CARD_LOGI("bk_sdio_host_discard_previous_receive_data_sema\r\n");
    ...
    SD_CARD_LOGI("bk_sdio_host_read_blks_fifo:0x%x, %d\r\n", data, block_num);

    if(rw_state == SDCARD_RW_STATE_READING)
        error_state = bk_sdio_host_continious_read_blks_fifo(data, block_num);
    else
