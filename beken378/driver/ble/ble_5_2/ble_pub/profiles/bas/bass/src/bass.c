...
__STATIC void bass_exe_operation(void)
{
    ...
    if((p_bass_env != NULL) && (!p_bass_env->in_exe_op))
    {
        ...
        while(!common_list_is_empty(&(p_bass_env->wait_queue)) && !(p_bass_env->op_ongoing))
        {
            uint16_t status = GAP_ERR_NO_ERROR;
            common_buf_t* p_buf = (common_buf_t*) common_list_pop_front(&(p_bass_env->wait_queue));
            bass_buf_meta_t* p_buf_meta = (bass_buf_meta_t*) common_buf_metadata(p_buf);
...
__STATIC uint16_t bass_notify_batt_lvl(bass_env_t *p_bass_env, uint8_t conidx, uint8_t svc_idx)
{
    ...
    uint16_t status = common_buf_alloc(&p_buf, GATT_BUFFER_HEADER_LEN, 1, GATT_BUFFER_TAIL_LEN);
    ...
    if(status == COMMON_BUF_ERR_NO_ERROR)
    {
        ...
        common_buf_data(p_buf)[0]       = p_bass_env->batt_lvl[svc_idx];

        // put event on wait queue
        common_list_push_back(&(p_bass_env->wait_queue), &(p_buf->hdr));
        // execute operation
        bass_exe_operation();
    }
    else
                                   ...
                                   uint16_t max_length)
{
    ...
    if(common_buf_alloc(&p_buf, GATT_BUFFER_HEADER_LEN, 0, PRF_CHAR_PRES_FMT_SIZE + GATT_BUFFER_TAIL_LEN) != GAP_ERR_NO_ERROR)
    {
 ...
 ****************************************************************************************
 */
__STATIC int bass_enable_req_handler(kernel_msg_id_t const msgid, struct bass_enable_req const *p_param,
                                     kernel_task_id_t const dest_id, kernel_task_id_t const src_id)
{
    ...
    p_rsp         = KERNEL_MSG_ALLOC(BASS_ENABLE_RSP, src_id, dest_id, bass_enable_rsp);
    if(p_rsp)
    {
 ...
 ****************************************************************************************
 */
__STATIC int bass_batt_level_upd_req_handler(kernel_msg_id_t const msgid,
                                            ...
                                            kernel_task_id_t const src_id)
{
    ...
    if(status != GAP_ERR_NO_ERROR)
    {
        ...
        struct bass_batt_level_upd_rsp *p_rsp = KERNEL_MSG_ALLOC(BASS_BATT_LEVEL_UPD_RSP, src_id, dest_id,
...
void bass_cb_batt_level_upd_cmp(uint16_t status)
{
    struct bass_batt_level_upd_rsp *p_rsp =
            KERNEL_MSG_ALLOC(BASS_BATT_LEVEL_UPD_RSP, PRF_DST_TASK(BASS), PRF_SRC_TASK(BASS), bass_batt_level_upd_rsp);
...
void bass_cb_bond_data_upd(uint8_t conidx, uint8_t ntf_ind_cfg)
{
    ...
    struct bass_batt_level_ntf_cfg_ind* p_ind =
            KERNEL_MSG_ALLOC(BASS_BATT_LEVEL_NTF_CFG_IND, PRF_DST_TASK(BASS), PRF_SRC_TASK(BASS), bass_batt_level_ntf_cfg_ind);
                          ...
                          struct bass_db_cfg *p_params, const bass_cb_t* p_cb)
{
    ...
    do
    {
        ...
        p_bass_env = (bass_env_t *) kernel_malloc(sizeof(bass_env_t), KERNEL_MEM_ATT_DB);
        ...
        if(p_bass_env != NULL)
        {
            ...
            common_list_init(&(p_bass_env->wait_queue));
...
__STATIC uint16_t bass_destroy(prf_data_t *p_env, uint8_t reason)
{
    ...
    if(status == GAP_ERR_NO_ERROR)
    {
        if(reason != PRF_DESTROY_RESET)
        {
            ...
            while(!common_list_is_empty(&p_bass_env->wait_queue))
            {
                common_buf_t* p_buf = (common_buf_t*) common_list_pop_front(&p_bass_env->wait_queue);
                common_buf_release(p_buf);
            }
 ...
 ****************************************************************************************
 */
__STATIC void bass_con_create(prf_data_t *p_env, uint8_t conidx, const gap_con_param_t* p_con_param)
{
    bass_env_t *p_bass_env = (bass_env_t *) p_env->p_env;
