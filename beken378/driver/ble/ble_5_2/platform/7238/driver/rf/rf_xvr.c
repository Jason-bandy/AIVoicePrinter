...
#include "power_save_pub.h"         // rf hold bit set/clear
...
// Power table
static const int8_t RF_RPL_TX_PW_CONV_TBL[RPL_PWR_TBL_SIZE] __maybe_unused = {
        [0] = -23,
        [1] = -20,
        [2] = -17,
        [3] = -14,
...
void xvr_reg_tx_pwr_set(uint32_t tx_pwr)
{
	REG_WRITE(BLE_XVR_REG3F, tx_pwr);
}
...
void xvr_reg_init(void)
{
	...
	*((volatile unsigned long *)(0x01050200)) = 0x200;
	...
	if (ble_get_sys_mode() == DUT_FCC_MODE)
		REG_WRITE(BLE_XVR_REG24, 0x000E0782);
	else
	...
	REG_WRITE(BLE_XVR_REG2A, 0x12083E44); //guodong20211223<15:8>=0x3e
	...
	REG_WRITE(BLE_XVR_REG3B, 0x3F34104C); //<11:9>=2 by qunshan20201224 for rx; guodong20211216<3:2>=3
	REG_WRITE(BLE_XVR_REG3C, 0x10F55501); //guodong20211213<15:8>=0x55 guodong20220615<23:16>=0xF5
...
void rf_init(struct rwip_rf_api *api)
{
	...
	ble_radiopwrupdn0_set(0x00650050);
	#else
	...
	#if (BLE_PHY_CODED_SUPPORT)
	ble_radiopwrupdn2_set(0x00650050);
	#else
	...
	#if (BLE_PHY_CODED_SUPPORT)
	ble_radiopwrupdn3_set(0x00000050);
	#else
	...
	ble_radiotxrxtim0_set(0x00000a03);  //0x00001007
	...
	ble_radiotxrxtim1_set(0x00050503);
	...
	#if (BLE_PHY_CODED_SUPPORT)
	ble_radiotxrxtim2_set(0x3500200B); //0x35002020
	#else
	...
	#if (BLE_PHY_CODED_SUPPORT)
	ble_radiotxrxtim3_set(0x0C00000B);
	#else
