...
#define MAX_SKIP_CON 4
uint32_t ble_switch_skip_cnt = 0;
extern void lld_con_evt_infor(struct bk_ble_event_arbiter *g_infor);
...
{
    uint8_t skip_max = MAX_SKIP_CNT;
    struct bk_ble_event_arbiter ble_infor = {0};
    ...
    if (txl_cntrl_pck_get() && (ble_switch_skip_cnt < skip_max))
    {
        ble_switch_skip_cnt++;
        GLOBAL_INT_RESTORE();
        return;
    ...
    if ((ps_get_sleep_prevent() & 0x35) && (ble_switch_skip_cnt < skip_max))
    {
        ble_switch_skip_cnt++;
        GLOBAL_INT_RESTORE();
        return;
