		...
		while (1) {
			if (nxmac_status_idle_interrupt_getf() == 1)
				...
				if (i_tmp > 1000) {
					...
					y_tmp ++;
					if (y_tmp > 50) {
						FS_FATAL("[mac] goto idle timeout\r\n");

						#if CONFIG_RECOVER_FORCE_IDLE_ISSUE
						/*TODO: root reason need be explored.if not reset mac fsm 
						 * system will hang / dead while here;
						 */
