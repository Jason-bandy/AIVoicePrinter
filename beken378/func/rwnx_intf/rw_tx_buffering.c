...
extern uint32_t rwm_transfer_node(MSDU_NODE_T *node, uint8_t flag);
/*
...
#if 0
static bool rwm_is_client_bufing_data(uint8_t client_idx)
{
    if ((client_idx >= MAX_BUFING_CLIENT_NUM) || (g_tx_bufing.tx_bufing_en == false))
    {
        return false;
    }

    if (g_tx_bufing.bufing_go_sap[client_idx].cntrl_src == 0)
    {
        return false;
    }
...
void rwm_tx_bufing_init(void)
{
    ...
    for( int i=0; i < MAX_BUFING_CLIENT_NUM; i++)
    {
        ...
        if(0 == i)
        {
            ...
            ret = rtos_init_timer(&g_tx_bufing.bufing_gc_sta.timer,
                                   MAX_BUFFER_TIME,
        ...
        ret = rtos_init_timer(&g_tx_bufing.bufing_go_sap[i].timer,
                               MAX_BUFFER_TIME,
