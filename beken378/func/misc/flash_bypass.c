...
static bk_err_t flash_bypass_otp_lock(flash_bypass_otp_ctrl_t *param)
{
    bk_err_t ret = BK_OK;
    uint8_t op_code = CMD_FLASH_WR_EN;
...
bk_err_t flash_bypass_get_otp_lock_bit(flash_bypass_otp_ctrl_t *param)
{
    bk_err_t ret = BK_OK;
    flash_bypass_otp_ctrl_t *lock_cfg = param;
    uint8_t  idx = lock_cfg->otp_idx;
...
bk_err_t flash_bypass_otp_operation(flash_bypass_otp_cmd_t cmd, flash_bypass_otp_ctrl_t *param)
{
    ...
    uint32_t op_len_max = OTP_LEN_MAX;
    ...
    case CMD_OTP_WRITE:
        ...
        for (uint8_t i = 0; i < (write_len / OTP_WRITE_LEN_MAX); i++) {
            if ((write_len - (i * OTP_WRITE_LEN_MAX)) <= OTP_WRITE_LEN_MAX) {
                ...
                otp_cfg->write_len   = OTP_WRITE_LEN_MAX;
                otp_cfg->addr_offset = offset + (i * OTP_WRITE_LEN_MAX);
            }
...
static uint32_t g_flash_id = 0xeb6015;
#elif (CFG_SOC_NAME == SOC_BK7238)
static uint32_t g_flash_id = 0x852015;
#elif (CFG_SOC_NAME == SOC_BK7252N)
static uint32_t g_flash_id = 0xeb6015;
#else
static uint32_t g_flash_id = 0x0;
#endif
