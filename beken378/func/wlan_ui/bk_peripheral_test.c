...
static void i2c1_test_eeprom(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	for (i = 0; i < I2C_TEST_EEPROM_LEGNTH; i++)
		BK_LOGI(TAG, "pData[%d]=0x%x\r\n", i, i2c_msg_config.pData[i]);

	ddev_close(i2c_hdl);

	BK_LOGI(TAG, " i2c2 test over\r\n");
...
static void i2c2_test_eeprom(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	BK_LOGI(TAG, " i2c2_test_eeprom start  \r\n");
...
void gspi_test(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	if (os_strcmp(argv[1], "master") == 0) {
		...
		BK_LOGI(TAG, "spi dma rx: rx_len:%d\n", rx_len);
		...
		if (buf) {
			...
			if (ret == 0) {
				for (int i = 0; i < rx_len; i++) {
					BK_LOGI(TAG, "%02x,", msg.recv_buf[i]);
					if ((i + 1) % 32 == 0)
						BK_LOGI(TAG, "\r\n");
				}
				BK_LOGI(TAG, "\r\n");
				...
				BK_LOGI(TAG, "spi dma recv error%d\r\n", ret);
		}
		...
		BK_LOGI(TAG, "spi dma tx: tx_len:%d,%d\n", tx_len, max_hz);
		...
		if (buf) {
			...
			if (ret == 0) {
				for (int i = 0; i < tx_len; i++) {
					BK_LOGI(TAG, "%02x,", msg.send_buf[i]);
					if ((i + 1) % 32 == 0)
						BK_LOGI(TAG, "\r\n");
				}
				BK_LOGI(TAG, "\r\n");
				...
				BK_LOGI(TAG, "spi dma send error%d\r\n", ret);
		}
		...
		BK_LOGI(TAG, "spi master  dma tx: tx_len:%d max_hz:%d\r\n", tx_len, max_hz);
		...
		BK_LOGI(TAG, "spi master  dma rx: rx_len:%d max_hz:%d\r\n\n", rx_len,max_hz);
		...
		if (buf) {
			...
			if (ret == 0) {
				...
				BK_LOGI(TAG, "spi dma rx error%d\r\n", ret);
		}
		...
		BK_LOGI(TAG, "gspi_test master/slave	 tx/rx	rate  len\r\n");

	//BK_LOGI(TAG, "cfg:%d, 0x%02x, %d\r\n", cfg->data_width, cfg->mode, cfg->max_hz);
...
void spi_Command(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	{
		BK_LOGI(TAG, "spi dma rx loop test\r\n");
