...
#define READ_REQ_CHARACTERISTIC_128         {0x04,0xFF,0,0,0x34,0x56,0,0,0,0,0x28,0x37,0,0,0,0}
...
enum
{
	...
	TEST_IDX_FF04_VAL_CHAR,
	TEST_IDX_FF04_VAL_READ,
	TEST_IDX_NB,
};
...
const bk_attm_desc_t test_att_db[TEST_IDX_NB] = {
	...
	//  Level Characteristic Declaration
	[TEST_IDX_FF04_VAL_CHAR]    = {BK_ATT_DECL_CHARACTERISTIC_128,  BK_PERM_SET(RD, ENABLE), 0, 0},
	///RD_MASK,WRITE_REQ_MASK  RI_POS
	[TEST_IDX_FF04_VAL_READ]   = {READ_REQ_CHARACTERISTIC_128,BK_PERM_SET(RD, ENABLE), BK_PERM_SET(RI, ENABLE)|BK_PERM_SET(UUID_LEN, UUID_16), 128},
};
...
void timer_Command(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	uint8_t channel = 0;
	uint32_t timer, read_cnt;

	channel = os_strtoul(argv[1], NULL, 10);
	timer = os_strtoul(argv[2], NULL, 10);

	if (os_strcmp(argv[3], "start") == 0) {

		os_printf("start timer test: %x, %x \r\n", channel, timer);
		bk_timer_initialize(channel, timer, timer_handler);

	} else if (os_strcmp(argv[3], "read") == 0) {

		read_cnt = bk_get_timer_cnt(channel);
		os_printf("read_cnt =%x\r\n", read_cnt);
	} else if (os_strcmp(argv[3], "stop") == 0) {

		bk_timer_stop(channel);
		os_printf("stop timer: %x\r\n", channel);
	} else

		os_printf("timer test param error \r\n");
