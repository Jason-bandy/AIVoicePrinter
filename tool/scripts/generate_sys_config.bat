...
tool\\gnuwin32\\sed.exe -n "/^#define/p" config/sys_config.h | tool\\gnuwin32\\awk.exe "{print $2}" | sort.exe | tool\\gnuwin32\\uniq.exe | tool\\gnuwin32\\awk.exe "{print """valueOf_"""$1"""="""$1}" >> config.c
echo # Autogenerated by Makefile, DON'T EDIT > .config
%ARM_GCC_TOOLCHAIN%arm-none-eabi-gcc -E config.c | tool\\gnuwin32\\grep.exe "^valueOf_" | tool\\gnuwin32\\sed.exe "s/valueOf_//" >> .config
tool\\gnuwin32\\sed.exe -i "/_SYS_CONFIG_H_/d" .config
rm -f config.c
